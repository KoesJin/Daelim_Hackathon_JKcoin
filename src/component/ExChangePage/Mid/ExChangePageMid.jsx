    ...
    const fetchCryptoData = async () => {
      ...
      try {
        ...
        filteredData.forEach((crypto) => {
          ...
          const prevPrice = priceChanges[crypto.id]
            ? priceChanges[crypto.id].price
            : priceUsd;
          const changeType =
            priceUsd > prevPrice
              ? "up"
        ...
        // 원래 순서 유지
        const sortedData = cryptoSymbols.map((symbol) =>
          filteredData.find((crypto) => crypto?.id === symbol)
        );
            ...
            {cryptoData.map((coin, index) => {
              ...
              return (
                <tr key={index}>
                  <td>
                    <Link
                      to={`/buycoins?coinName=${encodeURIComponent(coin.id)}`}
                    >
                  ...
                  <td>
                    {activeTab === "KRW"
                      ? `${(
                          (parseFloat(coin.volumeUsd24Hr) * exchangeRate) /
                          100000000
                        ).toFixed(2)}백만`
                      : `$${(parseFloat(coin.volumeUsd24Hr) / 100000).toFixed(
                          2
                        )}`}
                  </td>
                </tr>
